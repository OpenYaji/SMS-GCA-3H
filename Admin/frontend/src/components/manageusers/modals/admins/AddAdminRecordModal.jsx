...
export default function AddAdminRecordModal({
  show,
  onClose,
  onSuccess,
  adminEmail = "",
  ...
  }, [show, adminEmail, resetLocationData]);
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitError(null);
    setSubmitStatus("Creating Admin Records...");
    ...
    try {
      ...
      if (formData.step2.nationality?.trim()) {
        adminData.Nationality = formData.step2.nationality.trim();
      }

      adminData.AccountEmail = formData.step1.email.trim();

      // Add location codes for reference
      adminData.RegionCode = locationData.regionCode;
      adminData.ProvinceCode = locationData.provinceCode;
      adminData.CityCode = locationData.cityCode;
      adminData.BarangayCode = locationData.barangayCode;
      adminData.Region = locationData.region;
      adminData.Province = locationData.province;
      adminData.City = locationData.city;
      adminData.Barangay = locationData.barangay;
      adminData.StreetAddress = formData.step2.streetAddress.trim();
      ...
      Object.keys(adminData).forEach((key) => {
        if (key === "ProfilePicture") {
          formDataToSend.append(key, adminData[key]);
        } else {
          formDataToSend.append(key, adminData[key]);
        }
      });

      const response = await adminService.createAdmin(formDataToSend);
      ...
      <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50 backdrop-blur-md">
        ...
        >
          <div className="flex flex-col items-center text-center">
            ...
            >
              Admin record has been created successfully.
            </p>
            <button
              onClick={handleDone}
