    ...
    <div className="mb-1">
      ...
      <div className="relative">
        ...
        <input
          ...
          pattern={pattern}
          title={title}
          className={`w-full border rounded-lg pl-10 pr-3 py-2 focus:ring-2 outline-none ${
            darkMode
...
export default function AddGuardRecordModal({
  show,
  onClose,
  onSuccess,
  guardEmail = "",
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (formData.step2.nationality?.trim()) {
        guardData.Nationality = formData.step2.nationality.trim();
      }

      guardData.AccountEmail = formData.step1.email.trim();

      if (profilePicture) {
        guardData.ProfilePicture = profilePicture;
      }

      const formDataToSend = new FormData();
      Object.keys(guardData).forEach((key) => {
        if (key === "ProfilePicture") {
          formDataToSend.append(key, guardData[key]);
        } else {
          formDataToSend.append(key, guardData[key]);
        }
      });

      const response = await guardService.createGuard(formDataToSend);
      ...
      let errorMessage = "Failed to create guard. Please try again.";
  ...
  const getLoadingText = () => {
    switch (loadingStage) {
      case 1:
        return "Creating Guard Record...";
      case 2:
        return "Sending User Account...";
      default:
        return "Creating Guard Record...";
    }
  };
      ...
      <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50 backdrop-blur-md">
        ...
        >
          <div className="flex flex-col items-center text-center">
            ...
            >
              Guard record has been created successfully.
            </p>
            <button
              onClick={handleDone}
