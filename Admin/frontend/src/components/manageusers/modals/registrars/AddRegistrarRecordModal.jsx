...
export default function AddRegistrarRecordModal({
  show,
  onClose,
  onSuccess,
  registrarEmail = "",
  ...
  }, [show, registrarEmail, resetLocationData]);
  ...
  const handleStreetAddressChangeWrapper = (value) => {
    handleStreetAddressChange(value);
    setFormData((prev) => ({
      ...prev,
      step2: { ...prev.step2, streetAddress: value },
    }));
  };
  ...
  const validateStep1 = () => {
    ...
    if (!data.phone?.trim()) {
      ...
      const phoneValidation = inputValidation.validatePhoneWithFeedback(
        data.phone
      );
  ...
  const validateStep2 = () => {
    ...
    if (!locationData.regionCode) newErrors.regionCode = "Region is required";

    if (!hasNoProvinces && !locationData.provinceCode) {
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitError(null);
    setSubmitStatus("Creating Registrar Records...");

    try {
      // Simulate first step of creation process
      await new Promise((resolve) => setTimeout(resolve, 2000));

      setSubmitStatus("Sending User Account...");

      // Use the formatted address from location data
      ...
      if (formData.step2.nationality?.trim()) {
        registrarData.Nationality = formData.step2.nationality.trim();
      }

      registrarData.AccountEmail = formData.step1.email.trim();

      // Add location codes for reference
      registrarData.RegionCode = locationData.regionCode;
      registrarData.ProvinceCode = locationData.provinceCode;
      registrarData.CityCode = locationData.cityCode;
      registrarData.BarangayCode = locationData.barangayCode;
      registrarData.Region = locationData.region;
      registrarData.Province = locationData.province;
      registrarData.City = locationData.city;
      registrarData.Barangay = locationData.barangay;
      registrarData.StreetAddress = formData.step2.streetAddress.trim();

      // Add profile picture if uploaded
      if (profilePicture) {
        registrarData.ProfilePicture = profilePicture;
      }

      // Use FormData for file upload
      const formDataToSend = new FormData();
      Object.keys(registrarData).forEach((key) => {
        if (key === "ProfilePicture") {
          formDataToSend.append(key, registrarData[key]);
        } else {
          formDataToSend.append(key, registrarData[key]);
        }
      });

      const response = await registrarService.createRegistrar(formDataToSend);
      ...
      let errorMessage = "Failed to create registrar. Please try again.";
      ...
      <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50 backdrop-blur-md">
        ...
        >
          <div className="flex flex-col items-center text-center">
            ...
            >
              Registrar record has been created successfully.
            </p>
            <button
              onClick={handleDone}
            ...
            <div>
              ...
              <div className="mb-6">
                ...
                <AddressFields
                  ...
                  handleStreetAddressChange={handleStreetAddressChangeWrapper}
                  hasNoProvinces={hasNoProvinces}
                  darkMode={darkMode}
                />
