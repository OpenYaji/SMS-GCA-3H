...
export default function RegistrarInformationModal({
  isOpen,
  onClose,
  registrar,
  onRegistrarUpdate,
  ...
  const [registrarDetails, setRegistrarDetails] = useState(null);
  ...
  // Fetch single registrar details when modal opens
  const fetchRegistrarDetails = async () => {
    if (!registrar) return;

    setLoading(true);
    setApiError("");
    try {
      console.log(`Fetching registrar details for ID: ${registrar.id}`);
      const response = await fetch(
        `http://127.0.0.1:8000/api/v1/registrar-profiles/${registrar.id}`
      );
      ...
      console.log("Raw registrar details response:", data);

      if (!data) {
        throw new Error("No registrar data received from API");
      }
      const registrarData = data.data || data;
      if (!registrarData) {
        throw new Error("No registrar data found in API response");
      }

      const addressComponents = psgcService.parseAddress(
        registrarData.Profile?.Address || ""
      );

      const transformedData = {
        id: registrarData.RegistrarProfileID?.toString() || registrar.id,
        registrarId:
          registrarData.EmployeeNumber ||
          `R-${registrarData.RegistrarProfileID}`,
        name: `${registrarData.Profile?.FirstName || ""} ${
          registrarData.Profile?.LastName || ""
        }`.trim(),
        email: registrarData.User?.EmailAddress || "",

        // Personal Info
        firstName: registrarData.Profile?.FirstName || "",
        lastName: registrarData.Profile?.LastName || "",
        middleName: registrarData.Profile?.MiddleName || "",
        phone: registrarData.Profile?.PhoneNumber || "",
        ...
        nationality: registrarData.Profile?.Nationality || "Filipino",

        accountEmail: registrarData.User?.EmailAddress || "",

        // Store the full address for display
        fullAddress: registrarData.Profile?.Address || "",
        ...
        profilePicture: registrarData.Profile?.ProfilePictureURL || null,
        rawData: registrarData,
      };

      console.log("Transformed registrar data for modal:", transformedData);

      setRegistrarDetails(transformedData);
      setEditData(transformedData);
    } catch (error) {
      console.error("Error fetching registrar details:", error);
      setApiError(error.message);
      // Fallback to mock data if API fails
      const mockData = generateMockRegistrarDetails(registrar);
      setRegistrarDetails(mockData);
  ...
  useEffect(() => {
    if (registrarDetails && isEditing) {
      locationHook.setLocation({
        streetAddress: registrarDetails.streetAddress || "",
        regionCode: registrarDetails.regionCode || "",
        region: registrarDetails.region || "",
        provinceCode: registrarDetails.provinceCode || "",
        province: registrarDetails.province || "",
        cityCode: registrarDetails.cityCode || "",
        city: registrarDetails.city || "",
        barangayCode: registrarDetails.barangayCode || "",
        barangay: registrarDetails.barangay || "",
      });
    }
  }, [registrarDetails, isEditing]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const updateData = {
        FirstName: safe(editData.firstName, registrarDetails?.firstName),
        MiddleName:
          editData.middleName !== undefined
            ? editData.middleName
            : safe(registrarDetails?.middleName, ""),
        LastName: safe(editData.lastName, registrarDetails?.lastName),
      ...
      const result = await registrarService.updateRegistrar(
        registrar.id,
        updateData
      );
      ...
      const updatedRegistrar = {
        ...
        profilePicture:
          ...
          registrarDetails.profilePicture,
      };
      setRegistrarDetails(updatedRegistrar);

      if (onRegistrarUpdate) {
        onRegistrarUpdate(updatedRegistrar);
      }
  ...
  const displayData = isEditing ? editData : registrarDetails;
  const fullName = displayData
    ? formatName(displayData)
    : registrar?.name || "";
      ...
      >
          ...
          >
            ...
            >
              {saving ? (
                ...
                <>
                  <Check size={16} />
                  <span>Registrar information updated successfully!</span>
                </>
              )}
            </div>
          </div>
        ...
        <div className="p-5">
          {loading ? (
              ...
              <div className="flex flex-col md:flex-row gap-6 mb-6">
                <div className="flex flex-col md:flex-row gap-4 flex-1">
                  <div className="w-24 h-24 mx-auto md:mx-0 rounded-full flex-shrink-0 shadow-md overflow-hidden relative group">
                    ...
                    {imagePreview || registrarDetails?.profilePicture ? (
                      <img
                        src={imagePreview || registrarDetails.profilePicture}
                        alt={fullName}
                  ...
                  <div className="flex-1 text-center md:text-left">
                    ...
                    >
                      Registarar ID:{" "}
                      {registrarDetails?.registrarId || registrar?.id}
                    </p>
                    <p
                      className={`text-sm mb-1 ${
                        darkMode ? "text-gray-400" : "text-gray-600"
            ...
            >
              Failed to load registrar details.
            </div>
          )}
        </div>
        ...
        >
          {isEditing ? (
            ...
            <button
              onClick={handleEdit}
              disabled={saving || !registrarDetails}
...
// Mock data function
function generateMockRegistrarDetails(registrar) {
  if (!registrar) {
    return {
      firstName: "Registrar",
      middleName: "M.",
      lastName: "Manager",
      email: "registrar.manager@school.edu",
      ...
      city: "Quezon City",
      barangay: "Diliman",
      accountEmail: "registrar.manager@school.edu",
  ...
  const nameParts = registrar.name.split(" ").filter((part) => part.trim());
  let firstName = "";
  let middleName = "";
  let lastName = "";
