...
const StudentLogScreen = () => {
  ...
  const filteredStudentLog = useMemo(() => {
    if (!searchTerm) return studentLog;
    const lower = searchTerm.toLowerCase();
    return studentLog.filter(s => s.user && s.user.toLowerCase().startsWith(lower));
  }, [studentLog, searchTerm]);
  ...
  if (filteredStudentLog.length === 0 && studentLog.length > 0) {
    return (
  ...
  if (studentLog.length === 0) {
    return (
      <div className="text-center p-8 text-stone-700 bg-stone-100 border-2 border-stone-300 rounded-xl">
        ...
        <p className="text-sm">The database query returned no records from the `user_info` table in `aggabao_db`.</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-4 sm:p-0">
      ...
      <div className="bg-white rounded-2xl shadow-xl overflow-x-auto">
        <div className="grid grid-cols-5 text-left font-bold text-stone-900 bg-amber-300 p-4 rounded-t-2xl min-w-[500px]">
