...
try {
    ...
    // Required: Get specific School Year ID from query parameter for archive
    $schoolYearID = isset($_GET['schoolYearId']) ? $_GET['schoolYearId'] : null;
    ...
    if (empty($schoolYearID)) {
        ...
        echo json_encode([
            'success' => false, 
            'message' => 'SchoolYearID parameter is required for fetching archived students.'
        ]);
        exit;
    }
    ...
    // Base Query - fetches enrollment data for the SPECIFIC School Year ID
    $query = "
        SELECT 
            sp.StudentProfileID,
            sp.StudentNumber,
        ...
        -- Enrollment sa piniling School Year ID
        LEFT JOIN (
        ...
        WHERE u.IsDeleted = 0 
        -- Siguraduhin na ang kinuha lang na School Year ay INACTIVE (Ito ang fix)
        AND sy.IsActive = 0
    ";
