...
  // Handler for proceeding an applicant to the next stage (called from InboxView)
  const handleProceedToScreening = async (applicant) => {
    try {
      // Start removal animation state
      setRemovingIds((prev) => [...prev, applicant.id]);

      // API Call - Must be awaited to block further action in InboxView until complete
...
      
      // Remove applicant from UI after animation delay
      setTimeout(() => {
...
    } catch (err) {
        // Cancel removal animation state on error
        setRemovingIds((prev) => prev.filter((id) => id !== applicant.id)); 
        showToast(`Error updating stage: ${err.message}`, "error"); 
        // Re-throw the error so InboxView can catch it and reset its state
        throw err; 
    }
  };
