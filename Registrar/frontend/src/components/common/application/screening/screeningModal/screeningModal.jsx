...
const ScreeningModal = ({ applicant, onClose, onValidate, isProcessing }) => {
    ...
    useEffect(() => {
        // Fetches fee structure and sets fixed down payment when applicant or payment mode changes
        if (applicant?.grade) {
            const fees = getFeeStructure(applicant.grade);
            setFeeStructure(fees);
            // Set fixed downPayment based on the current paymentMode
            setDownPayment(fees.downPayments[paymentMode]);

            // Set installment amount based on payment mode
            if (paymentMode === "quarterly") {
                setInstallmentAmount(fees.quarterly);
            } else if (paymentMode === "monthly") {
                setInstallmentAmount(fees.monthly);
            } else {
    ...
    const getFeeStructure = (grade) => {
        ...
        return {
            ...fees,
            downPayments: {
                full: fees.registration + fees.miscellaneous + fees.tuition, // Full payment
    ...
    const handleValidateClick = async () => {
        ...
        // No validation needed - down payment is always correct based on mode
                    ...
                    {feeStructure && (
                        <div className="border-t pt-4 mt-4">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                {/* Payment Mode Selection and Down Payment Display */}
                                <div className="space-y-3">
                                    ...
                                    <div>
                                        ...
                                        {/* Read-only display of fixed down payment */}
                                        <div className="w-full border border-gray-400 dark:border-gray-600 rounded px-3 py-2 bg-gray-100 dark:bg-slate-600 text-sm font-semibold">
                                            ₱ {downPayment.toLocaleString()}
                                        </div>
                                        <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                                            Fixed amount based on selected payment mode
                                        </p>
                                    </div>
                                    ...
                                    <div className="bg-blue-50 dark:bg-blue-900 p-3 rounded">
                                        ...
                                        {paymentMode !== "full" && (
                                            <div className="flex justify-between text-sm mb-1">
                                                <span>{paymentMode === "quarterly" ? "Quarterly Payment:" : "Monthly Payment:"}</span>
                                                <span className="font-semibold text-blue-600 text-4xl dark:text-blue-300">
                                                    ₱ {installmentAmount.toLocaleString()}
                                                    <span className="text-sm ml-1">
                                                        ({paymentMode === "quarterly" ? "x4" : "x8"})
                                                    </span>
                                                </span>
                                            </div>
                                        )}
