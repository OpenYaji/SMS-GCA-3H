...
const DocumentPage = () => {
  ...
  const documentTypes = [
    { value: 'form137', label: 'Form 137 (Permanent Record)' },
    { value: 'grades', label: 'Certificate of Grades' },
    { value: 'goodMoral', label: 'Certificate of Good Moral' },
  ...
  const fetchRequestHistory = async () => {
    try {
      const response = await axios.get('/backend/api/documents/getRequests.php', {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        '/backend/api/documents/requestDocument.php',
        formData,
        {
          headers: {
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    >
                      ...
                      <option value="mail">Mail Delivery (+₱50)</option>
                    </select>
                  </div>
                </div>
              ...
              <table className="w-full">
                ...
                <tbody>
                  {requestHistory.map((request, index) => (
                    <tr key={index} className="border-b border-gray-100 dark:border-slate-700/50 hover:bg-gray-50 dark:hover:bg-slate-700/30 transition">
                      <td className="py-3 px-4 text-sm text-gray-600 dark:text-gray-400">#{request.id || '000'}</td>
                      <td className="py-3 px-4 text-sm text-gray-600 dark:text-gray-400">{request.documentType || 'N/A'}</td>
                      <td className="py-3 px-4 text-sm text-gray-600 dark:text-gray-400">{request.purpose || 'N/A'}</td>
                      <td className="py-3 px-4 text-sm text-gray-600 dark:text-gray-400">{request.date || 'N/A'}</td>
